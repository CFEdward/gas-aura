...
void UExecCalc_Damage::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	TargetBlockChance = FMath::Max<float>(TargetBlockChance, .0f);
	...
	SourceArmorPenetration = FMath::Max<float>(SourceArmorPenetration, .0f);

	// ArmorPenetration ignores a percentage of the Target's Armor
	const float EffectiveArmor = TargetArmor *= (100.f - SourceArmorPenetration * .25f) / 100.f;
	// Armor ignores a percentage of incoming Damage
	Damage *= (100.f - EffectiveArmor * .333f) / 100.f;
